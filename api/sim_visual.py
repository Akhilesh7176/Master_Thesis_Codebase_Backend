...
def sim(v0, T, s0, a, b, data):
    ...
    follower_data = data[['Follower', 'Time [s]', 'x1', 'y1', 'v1', 'Follower Vehicle Type']]
    follower_data['Follower Vehicle Type'] = follower_data['Follower Vehicle Type'].replace('Motorcycle', 'Medium Vehicle')

    follower_data.columns = ['vehicle_id', 'Time [s]', 'x_rot', 'y_rot', 'Speed [km/h]', 'flw_type']

    follower_data = data_antirot(follower_data)

    follower_data = follower_data[['vehicle_id', 'Time [s]', 'x[m]', 'y[m]', 'Speed [km/h]', 'flw_type']]
...
if __name__ == '__main__':
    ...
    l_data['flw_type'] = l_data['flw_type'].replace('Motorcycle', 'Medium Vehicle')
    ...
    out_data1 = out_data.sort_values(by=['vehicle_id', 'Time [s]'], ascending=True)

    # Add f_error as a column to be included in the CSV output
    out_data1['f_error'] = f_error

    out_data1.to_csv(
        fr'../../Master_Thesis_frontend/public/output_data.csv', index=False)
